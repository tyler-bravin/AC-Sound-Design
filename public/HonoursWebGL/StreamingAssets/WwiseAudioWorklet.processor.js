		...
		if (buffersReady > 0)
		{
			...
			for (var c = 0; c < this._channelCount; c++) {
				...
				if (this._channelCount == 6) {
					// 5.1 output; Wwise gave us the LFE channel at the end but WebAudio expect standard WAVE order, so read the input channels in this order: 0,1,2,5,3,4
					if ( c === 3 )
						cIn = 5;
					else if ( c > 3 )
						cIn -= 1;
				}
				const channelOffset = this._readIndex + (cIn * inlen) + this._frameIndex;
